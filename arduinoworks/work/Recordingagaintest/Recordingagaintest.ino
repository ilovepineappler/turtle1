...
void loop() {
    ...
    if(Serial.available()){                          //Send commands over serial to play
      switch(Serial.read()){
        case 'r': audio.startRecording("testr.wav",16000,A0);
        Serial.println("record");
        break;    //Record at 16khz sample rate on pin A0
        case 'R': audio.startRecording("testr.wav",16000,A0,1); break;  //Record, but with passthrough to speaker.
        case 't': audio.startRecording("testr.wav",16000,A0,2);
        Serial.println("recordout");
        break;  //Do not record. Output direct to speaker
        							       //Note: If samples are dropped before writing, it
        							       //      will not be heard in passthrough mode
        case 's': audio.stopRecording("testr.wav");
        Serial.println("recorstop");
        break;              //Stop recording
        case 'p': audio.play("testr.wav");
        Serial.println("recordplay");
        break;                       //Play the recording 
